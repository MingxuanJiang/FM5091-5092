...
namespace MonteCarloSim_ExoticOptions
{
    ...
    {
        public Asian(double S, double K, double Rate, double Sigma, double Tenor, int Sims, int Steps, bool TYPE, bool Ant, bool CV, bool Multithreading, double Rebate, double Barrier, int BarrierType)
        ...
        public override double[] OptionPrice()
        {
            ...
            if (Ant == true)//choose Ant Var
            {
                ...
                if (CV == true)//choose CV
                {
                    ...
                    for (int i = 0; i < 2 * Sims; i++)
                    {
                        ...
                        if (TYPE == true)
                        {
                            CT[i] = (Math.Max(AveragePrice[i] - K, 0) - cv) * Math.Exp(-Rate * Tenor);
                        }
                        else
                        {
                            CT[i] = (Math.Max(K - AveragePrice[i], 0) - cv) * Math.Exp(-Rate * Tenor);
                        }
                    }
                ...
                else//not choose CV
                {
                    ...
                    if (TYPE == true)//call
                    {
                        ...
                        {
                            value[i] = Math.Max(AveragePrice[i] - K, 0);
                            value[i + Sims] = Math.Max(AveragePrice[i + Sims] - K, 0);
                            sum1 += value[i];
                        }
                    }
                    else//put
                    {
                        for (int i = 0; i < Sims; i++)
                        {
                            value[i] = Math.Max(K - AveragePrice[i], 0);
                            value[i + Sims] = Math.Max(K - AveragePrice[i + Sims], 0);
                            sum1 += value[i];
                        }
                    }
            ...
            else//not choosing Ant Var
            {
                ...
                allsims = all.allsims_calculate(S, K, Rate, Sigma, Tenor, Sims, Steps, TYPE, Ant, CV, Multithreading, Rn);
                //calculate average price
                double[] AveragePrice = new double[Sims];
                for (int i = 0; i < Sims; i++)
                {
                    double SumPrice = 0;
                    for (int j = 0; j < Steps + 1; j++)
                ...
                else//not choose CV
                {
                    ...
                    {
                        for (int i = 0; i < Sims; i++)
                            value[i] = Math.Max(AveragePrice[i] - K, 0) * Math.Exp(-Rate * Tenor);
                    }
                    else//put
                    {
                        for (int i = 0; i < Sims; i++)
                            value[i] = Math.Max(K - AveragePrice[i], 0) * Math.Exp(-Rate * Tenor);
                    }
                    //calculate option price
